...
class DeepBGCpredDetector(PipelineStep):
    def __init__(self, detector, pfamfile, input_size=[102, 64, 64], sliding_window=True, sw_width=256, sw_steps=20,
                 label=None, pcfile=True, score_threshold=0.5,merge_max_protein_gap=0, merge_max_nucl_gap=0, min_nucl=1, min_proteins=1,
                 min_domains=1, min_bio_domains=0):
    ...
    def run(self, record):
        ...
        pfam_sequence[self.score_column] = self.model.predict(pfam_sequence, self.input_size, self.sliding_window, self.sw_width, self.sw_steps)
