...
class AntismashJSONWriter(OutputWriter):
    ...
    def _get_cluster_classes_str(self, cluster, classifier_name):
        class_str_list = cluster.qualifiers.get(
            util.format_classification_column(classifier_name)
        )
        return class_str_list[0] if class_str_list else "no confident class"

    def _create_cluster_json(self, cluster, classifier_names):
        classes = {
            cls_name: self._get_cluster_classes_str(cluster, cls_name)
    ...
    def _create_record_json(self, name, subregions, protoclusters):
        return {"name": name, "subregions": subregions, "protoclusters": protoclusters}

    def close(self):
        zipped_records = zip(
            self.record_ids, self.record_subregions, self.record_protoclusters
        )
        data = {
            "tool": {
                "name": "DeepBGC",
            ...
            "records": [
                self._create_record_json(name, sr, pc)
                for name, sr, pc in zipped_records
            ],
        }
