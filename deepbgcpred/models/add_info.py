...
class DeepBGCpredAddInfor:
    def __init__(
        self, clan_txt, prepared_tsv, output_new_tsv
    ):
    ...
    def add_information(self):
        ...
        for i in range(self.pfam_clan.shape[0]):
            pfam_clan_dict[self.pfam_clan.iloc[i]['pfamA_acc']] = self.pfam_clan.iloc[i]['clan_acc']
            pfam_desc_dict[self.pfam_clan.iloc[i]['pfamA_acc']] = self.pfam_clan.iloc[i]['description']
