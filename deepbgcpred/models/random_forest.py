...
class RandomForestClassifier(object):
    ...
    def predict(self, X):
        ...
        if len(X):
            predictions = np.array(
                [class_pred[:, 1] for class_pred in self.rf.predict_proba(X)]
            ).T
        else:
            predictions = []
    ...
    def fit(
        ...
        if not isinstance(X_list, pd.DataFrame):
            raise ValueError(
                "Random Forest expects samples in a single pd.DataFrame, got {}".format(
                    type(X_list)
                )
            )
        if not isinstance(y_list, pd.DataFrame):
            raise ValueError(
                "Random Forest expects targets in a single pd.DataFrame, got {}".format(
                    type(X_list)
                )
            )
        ...
        for target in self.targets_:
            ...
            if target_values != {0, 1}:
                raise ValueError(
                    "Classifier column should contain (0, 1) values, "
                    "got: {} in {}".format(target_values, target),
        ...
        if sys.version_info[0] == 3:
            logging.warning(
                "Warning! Random Forests trained with Python 3 will not be readable in Python 2! "
    ...
    def get_feature_importances(self):
        return pd.Series(self.rf.feature_importances_, index=self.inputs_).sort_values(
            ascending=False
        )
