...
def read_pfam_csv(path):
    ...
    if 'evalue' in df.columns:
        ...
        raise ValueError('Old Pfam CSV format with the "evalue" column is not supported anymore. Create the Pfam CSV again using "deepbgcpred pfam" command or filter on e-value yourself and remove the column.')
    return df
