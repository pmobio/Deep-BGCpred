...
SCORE_SUFFIX = "_score"
STRUCTURED_COMMENT_DETECTOR_PREFIX = "deepbgcpred_detector_"
STRUCTURED_COMMENT_CLASSIFIER_PREFIX = "deepbgcpred_classifier_"
...
def get_proteins_by_id(protein_features):
    return {
        ...
        for protein_id in get_protein_ids(feature)
    }
...
FEATURE_ORDER = {"cluster": -3, "gene": -2, "CDS": -1}
...
def sort_record_features(record):
    record.features = sorted(
        ...
        key=lambda feature: (
            ...
            FEATURE_ORDER.get(feature.type, 0),
        ),
    )
...
def get_pfam_features(record):
    ...
    from deepbgcpred.data import PFAM_DB_VERSION

    filtered_pfams = [
        pfam for pfam in pfams if pfam.qualifiers.get("database") == [PFAM_DB_VERSION]
    ]
    num_incompatible = len(pfams) - len(filtered_pfams)
    if num_incompatible:
        # TODO: enable users to run model with older incompatible Pfam DB versions?
        logging.warning(
            'Ignoring {} incompatible Pfam features with database different than "{}"'.format(
                num_incompatible, PFAM_DB_VERSION
            )
        )
...
def get_cluster_features(record, detector=None):
    ...
    if detector:
        features = [
            f for f in features if f.qualifiers.get("detector", [None])[0] == detector
        ]
    return features
...
def get_protein_ids(feature):
    ...
    if not ids:
        raise NotImplementedError(
            "No recognized protein ID for feature: {}".format(feature)
        )
    return ids
...
def get_record_detector_meta(record):
    ...
    return {
        value["label"]: value
        for comment_key, value in comments.items()
        if comment_key.startswith(STRUCTURED_COMMENT_DETECTOR_PREFIX)
    }


def get_record_detector_names(record):
    return sorted(
        list(set([meta["name"] for meta in get_record_detector_meta(record).values()]))
    )
...
def get_record_classifier_meta(record):
    ...
    return {
        value["name"]: value
        for comment_key, value in comments.items()
        if comment_key.startswith(STRUCTURED_COMMENT_CLASSIFIER_PREFIX)
    }
...
def get_record_classifier_names(record):
    return sorted(
        list(
            set([meta["name"] for meta in get_record_classifier_meta(record).values()])
        )
    )
...
def extract_cluster_record(cluster_feature, record):
    ...
    cluster_record.id = cluster_feature.qualifiers.get(
        "bgc_candidate_id", ["unknown_cluster_id"]
    )[0]
    ...
    cluster_record.features = [
        f
        for f in cluster_record.features
...
def create_pfam_dataframe(record, add_scores=True, add_in_cluster=False):
    ...
    cluster_locations = (
        [f.location for f in get_cluster_features(record, "annotated")]
        if add_in_cluster
        else []
    )
    df = create_pfam_dataframe_from_features(
        pfam_features, proteins_by_id, detector_names, cluster_locations
    )
...
def create_pfam_dataframe_from_features(
    pfam_features, proteins_by_id, detector_names=[], cluster_locations=[]
):
    return pd.DataFrame(
        [
            create_pfam_dict(pfam, proteins_by_id, detector_names, cluster_locations)
...
def create_pfam_dict(pfam, proteins_by_id, detector_names, cluster_locations):
    ...
    if locus_tag not in proteins_by_id:
        ...
        raise ValueError(
            'Got pfam with missing protein ID "{}": {}'.format(locus_tag, pfam)
        )
    ...
    if cluster_locations:
        pfam_dict["in_cluster"] = int(
            any(pfam.location.start in loc for loc in cluster_locations)
        )
...
def create_cluster_dataframe(
    record, add_pfams=True, add_proteins=True, add_classification=False
):
    # FIXME: make sure columns are provided even with an empty DataFrame to avoid KeyErrors down the line
...
def create_cluster_dict(
    cluster, record, add_pfams=True, add_proteins=True, classifier_names=[]
):
    ...
    cluster_dict["detector_version"] = cluster.qualifiers.get(
        "detector_version", [None]
    )[0]
    cluster_dict["detector_label"] = cluster.qualifiers.get(
...
def get_downloaded_file_path(filename, dirname="", check_exists=True, versioned=True):
    ...
    if check_exists and not os.path.exists(path):
        ...
        raise ValueError(
            'File "{}" is not downloaded yet'.format(filename),
            'Use "deepbgcpred download" to download all dependencies',
            dir_msg,
        )
    return path
...
def download_files(downloads):
    ...
    for download in downloads:
        ...
        if dirname:
            dirpath = get_downloaded_file_path(
                filename="", dirname=dirname, check_exists=False, versioned=versioned
            )
            if not os.path.exists(dirpath):
                os.makedirs(dirpath)
        target_path = get_downloaded_file_path(
            filename, dirname=dirname, check_exists=False, versioned=versioned
        )
        logging.info("=" * 80)
        download_file(
            url=url, target_path=target_path, checksum=checksum, gzipped=gzipped
        )
...
def download_file(url, target_path, checksum, gzipped=False):
    ...
    if downloaded_checksum != checksum:
        raise ValueError(
            'File MD5 checksum "{}" does not match expected checksum "{}". Please try again.'.format(
...
def get_available_models(model_type):
    available_paths = glob.glob(
        os.path.join(get_downloaded_models_dir(model_type), "*.pkl")
    )
...
def get_model_path(model_name, model_type):
    ...
    if not os.path.exists(path):
        ...
        if available_names:
            raise ValueError(
                'Model "{}" does not exist in path {}'.format(model_name, path),
                "Available models: {}".format(", ".join(available_names)),
            )
        raise ValueError(
            'DeepBGC models directory does not exist yet: "{}". '.format(model_dir),
            'Run "deepbgcpred download" to download trained models',
...
def run_hmmpress(path, force=False):
    ...
    if not find_executable("hmmpress"):
        raise RuntimeError(
            "HMMER hmmpress needs to be installed and available on PATH "
...
def fix_record_locus(record):
    ...
    if record.name and len(record.name) > MAX_LEN:
        ...
        logging.warning(
            'Trimming long record locus "{}" to "{}"'.format(record.name, new_locus)
        )
...
def fix_duplicate_cds(record):
    ...
    for feature in get_protein_features(record):
        ...
        if protein_id != new_protein_id:
            logging.warning(
                "Setting new unique_protein_id %s for CDS %s",
                new_protein_id,
                protein_id,
            )
...
def read_pfam_csv(path):
    ...
    if "pfam_id" not in df.columns:
        raise ValueError(
            'File is not a Pfam CSV sequence, missing "pfam_id" column: {}'.format(path)
        )
    if "evalue" in df.columns:
        ...
        raise ValueError(
            'Old Pfam CSV format with the "evalue" column is not supported anymore. Create the Pfam CSV again using "deepbgcpred pfam" command or filter on e-value yourself and remove the column.'
        )
    return df
...
def read_samples(paths, target_column=None):
    ...
    for sample_path in paths or []:
        ...
        if fmt == "csv":
            ...
            if target_column and target_column not in domains.columns:
                raise ValueError(
                    'Sample does not contain target column "{}": {}'.format(
                        target_column, sample_path
                    )
                )
        elif fmt == "genbank":
            ...
            if target_column and not target_in_cluster:
                raise NotImplementedError(
                    'Only "in_cluster" target is supported '
                    "when using GenBank samples: {}".format(sample_path)
                )
            domains = pd.concat(
                [
                    create_pfam_dataframe(
                        record, add_scores=False, add_in_cluster=target_in_cluster
                    )
            ...
            raise NotImplementedError(
                "Samples have to be provided in Pfam TSV or annotated GenBank format, got: {}".format(
                    sample_path
                )
            )
        samples = [sample for sample_id, sample in domains.groupby("sequence_id")]
        logging.info(
            "Loaded %s samples and %s domains from %s",
            len(samples),
            len(domains),
            sample_path,
        )
    ...
    if len(unique_y) == 1:
        raise ValueError(
            "Got target variable with only one value {} in: {}".format(unique_y, paths),
...
def read_samples_with_classes(sample_paths, classes):
    ...
    if not len(common_sample_ids):
        raise ValueError(
            "No overlap found between classes and samples. Classes should be indexed by sequence_id."
        )
    ...
    if num_missing:
        logging.warning(
            "Warning: Removing %s/%s samples with missing class from %s",
            num_missing,
            num_total,
            sample_paths,
        )
    samples_with_class = [
        samples.get_group(sample_id) for sample_id in common_sample_ids
    ]
...
def guess_format(file_path, accept_csv=False):
    ...
    raise NotImplementedError(
        "Sequence file type not recognized: '{}', ".format(file_path),
...
def is_valid_hmmscan_output(domtbl_path):
    ...
    if "# [ok]" not in line:
        logging.warning(
            "Not using existing but incomplete HMMER hmmscan output: %s", domtbl_path
        )
        return False
    return True
...
class SequenceParser(object):
    ...
    def parse(self):
        ...
        if self.protein:
            if self.fmt != "fasta":
                raise ValueError(
                    "Only FASTA format can be parsed in --protein mode, got: {}".format(
                        self.file_path
                    )
                )
